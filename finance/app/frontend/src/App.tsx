...
export default function App() {
  ...
  else if(page==='dashboard') content=<Dashboard accounts={accounts} transactions={transactions} categories={categories} projects={projects} period={period} range={customRange} onPeriodChange={setPeriod} onRangeChange={setCustomRange} onNavigate={setPage} onCreateTransaction={()=>{setQuickTransaction(true);setPage('transactions')}} onOpenBudget={id=>{setBudgetToOpen(id);setPage('budgets')}} onOpenProject={id=>{setProjectToOpen(id);setPage('budgets')}}/>;
  else if(page==='analytics') content=<Analytics accounts={accounts} categories={categories} period={period} range={customRange} onPeriodChange={setPeriod} onRangeChange={setCustomRange}/>;
  ...
  else if(page==='transactions') content=<Transactions transactions={transactions} accounts={accounts} categories={categories} projects={projects} period={period} range={customRange} taxApiEnabled={taxApiEnabled} quickCreate={quickTransaction} onQuickCreateHandled={()=>setQuickTransaction(false)} onSettings={()=>setPage('settings')} onPeriodChange={setPeriod} onRangeChange={setCustomRange} onChanged={()=>void load(false)}/>;
  else if(page==='budgets') content=<PlanningHub accounts={accounts} categories={categories} projects={projects} period={period} range={customRange} refreshKey={dataVersion} initialBudgetId={budgetToOpen} onInitialBudgetOpened={()=>setBudgetToOpen(null)} initialProjectId={projectToOpen} onInitialProjectOpened={()=>setProjectToOpen(null)} onBack={()=>setPage('dashboard')} onChanged={()=>void load(false)}/>;
