...
export function Categories({ categories, onBack, onChanged }: { categories: Category[]; onBack: () => void; onChanged: () => void }) {
  ...
  function renderChildren(parentId: string, depth = 1): React.ReactNode { return categories.filter(category => category.parentId === parentId && category.type === activeType).map(child => <div key={child.id}><div className="category-child" style={{ paddingLeft: `${24 + depth * 14}px` }}><CategoryIcon name={child.icon} color={child.color} />{child.name}<span className="category-spacer" /><button className="category-delete" onClick={() => openEdit(child)} aria-label={`Редагувати ${child.name}`}><Pencil size={14} /></button><button className="category-delete" onClick={() => void removeCategory(child)} aria-label={`Видалити ${child.name}`}><Trash2 size={14} /></button></div>{renderChildren(child.id, depth + 1)}</div>); }
