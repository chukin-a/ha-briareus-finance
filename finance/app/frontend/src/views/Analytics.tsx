...
export function Analytics({ accounts, categories, period, range, onPeriodChange, onRangeChange }: { accounts:Account[];categories:Category[];period:PeriodPreset;range:CustomRange;onPeriodChange:(period:PeriodPreset)=>void;onRangeChange:(range:CustomRange)=>void }) {
  const [data,setData]=useState<AnalyticsData|null>(null),[error,setError]=useState(''),[drillCategory,setDrillCategory]=useState<string|null>(null),[chartTab,setChartTab]=useState<'expense'|'income'>('expense'),[selectedTrend,setSelectedTrend]=useState<string|null>(null);
  ...
  const selectedTrendRow=trendRows.find(row=>row.date===selectedTrend);
  ...
  return <main className="screen analytics-screen"><header className="screen-header"><div><span className="eyebrow">ФІНАНСОВИЙ ОГЛЯД</span><h1>Аналітика</h1></div><div className="round-action"><ChartPie/></div></header><div className="period"><PeriodPicker value={period} range={range} onChange={onPeriodChange} onRangeChange={onRangeChange}/></div>{error&&<div className="category-error">{error}</div>}{!data?<p className="empty">Завантаження…</p>:<>
    ...
    <section className="section-title"><h2>Динаміка {trendCurrency}</h2></section><div className="analytics-trend-wrap"><div className="analytics-trend">{trendRows.map(row=><div role="button" tabIndex={0} onClick={()=>setSelectedTrend(row.date)} key={`${row.date}:${row.currency}`}><span title={`Прибутки: ${money(row.incomeMinor,row.currency)}`} style={{height:`${Math.max(2,row.incomeMinor/dailyMax*100)}%`}}/><i title={`Витрати: ${money(row.expenseMinor,row.currency)}`} style={{height:`${Math.max(2,row.expenseMinor/dailyMax*100)}%`}}/><time>{row.date.slice(5)}</time></div>)}</div>{selectedTrendRow&&<div className="analytics-trend-detail"><button type="button" aria-label="Закрити значення" onClick={()=>setSelectedTrend(null)}>×</button><strong>{selectedTrendRow.date}</strong><span className="income">Прибутки: {money(selectedTrendRow.incomeMinor,selectedTrendRow.currency)}</span><span>Витрати: {money(selectedTrendRow.expenseMinor,selectedTrendRow.currency)}</span></div>}</div>
