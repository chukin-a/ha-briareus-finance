...
export function Transactions({ transactions, accounts, categories, projects, period, range, onPeriodChange, onRangeChange, onChanged }: { transactions: Transaction[]; accounts: Account[]; categories: Category[]; projects: Project[]; period: PeriodPreset; range: CustomRange; onPeriodChange: (p: PeriodPreset) => void; onRangeChange: (range: CustomRange) => void; onChanged: () => void }) {
  ...
  const reset = () => { setAmount(''); setDescription(''); setCategoryId(''); setProjectId(''); setError(''); setEditing(null); setOpen(false); };
  ...
  return <main className="screen">
    ...
    <div className="transactions full-transactions">{!visible.length && <p className="empty">Операцій за цим періодом немає</p>}{visible.map(transaction => <article className="transaction" key={transaction.id}><time>{shortDate(transaction.occurredAt)}</time><div className={`transaction-icon ${transaction.type}`}>{transaction.type === 'transfer' ? '↔' : transaction.type === 'income' ? '+' : '−'}</div><div className="transaction-copy"><strong>{transaction.description || categories.find(c => c.id === transaction.categoryId)?.name || (transaction.type === 'income' ? 'Надходження' : transaction.type === 'transfer' ? 'Переказ' : 'Витрата')}</strong><span>{accounts.find(a => a.id === transaction.accountId)?.name || 'Рахунок'}</span></div><b className={transaction.type === 'income' ? 'income' : ''}>{transaction.type === 'income' ? '+' : '−'} {money(transaction.amountMinor, transaction.currency)}</b>{transaction.type !== 'transfer' && <button className="transaction-edit" aria-label="Редагувати операцію" onClick={() => edit(transaction)}><Pencil size={16} /></button>}<button className="transaction-delete" onClick={() => void remove(transaction)}><Trash2 size={16} /></button></article>)}</div>
