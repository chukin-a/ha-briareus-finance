...
export function Transactions({ transactions, accounts, categories, projects, period, range, onPeriodChange, onRangeChange, onChanged, initialReceipt, onInitialReceiptConsumed }: { transactions: Transaction[]; accounts: Account[]; categories: Category[]; projects: Project[]; period: PeriodPreset; range: CustomRange; onPeriodChange: (p: PeriodPreset) => void; onRangeChange: (range: CustomRange) => void; onChanged: () => void; initialReceipt?: ReceiptPrefill | null; onInitialReceiptConsumed: () => void }) {
  ...
  async function scanTaxQr(data: TaxQrData) { try { const extracted = await financeApi.lookupTaxReceipt(data); setType('expense'); setAmount(extracted.amountMinor === null ? '' : String(extracted.amountMinor / 100).replace('.', ',')); setDescription(extracted.description || extracted.merchant || ''); setDate(extracted.occurredOn || new Date().toISOString().slice(0, 10)); setCategoryId(''); setProjectId(''); setTags(extracted.tags.join(', ')); setMetadata({ ...extracted.metadata, url: data.url }); setReceiptId(null); setScannerOpen(false); setError(''); setOpen(true); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Не вдалося отримати чек з ДПС'); } }
